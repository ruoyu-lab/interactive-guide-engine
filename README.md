...
项目当前包含框架无关的核心 engine、可替换的 DOM adapter、基础 DOM renderer、可选 DOM effects，以及一个 Vue demo。核心包不绑定游戏、节点编辑器、交易系统、后台系统或任何特定业务领域。
...
如果只使用核心 engine，可以不引入 DOM adapter、DOM renderer、DOM effects 和 CSS。只要使用 `DomTutorialRenderer`，就需要引入 `interactive-guide-engine/dom-renderer/style.css`，否则遮罩、高亮和气泡没有默认样式。`DomTutorialEffects` 会自动注入自己的动画样式，并支持内置或自定义光标样式。
...
let effects: DomTutorialEffects | undefined
...
packages/
  ...
  dom-effects/            # 可选 DOM 动画层，播放步骤 effects
